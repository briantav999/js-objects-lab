...
*/
game.party.push(pokemon[5], pokemon[34], pokemon[54])
...
// console.log("exercise 6" + game.party + "ends here");
...
*/
let gymStatus = () => {
   let gymTally = { //setting up gymTally object
    completed: 0,
    incomplete: 0
    }
    for(let i=0;i< game.gyms.length;i++) { // for loop to iterate through game.gyms array
        if (game.gyms[i].completed = true){ // if conditional, if the gym is completed the gymTally will update its completed property to increment by 1
            gymTally.completed++
        } else { // other wise the incomplete property gets incremented.
            gymTally.incomplete++
        }
